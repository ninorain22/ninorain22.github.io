...
![avatar](/assets/images/linux_进程内核栈.png)
...
![avatar](/assets/images/linux_进程状态转化.png)
...
休眠有2种相关的进程状态：TASK_INTERRUPTIBLE和TASK_UNINTERRUPTIBLE。
...
也可以采取让请求进程睡眠，然后直到锁可用时再唤醒它，这样处理器不必等待，但是实现复杂度较高，有明显的两次上下文切换。
...
+ 由于争用信号量的进程在等待锁重新可用时会水木，所以信号量适合锁会被长时间持有的情况
...
读-写信号量和读-写自旋锁一样，除非代码中的读和写可以明白的区分开，否则最好不要使用它。
...
slab分配器可以创建新的slab，通过低级内核页分配器进行__get_free_pages()
...
## 13.4 VFS对象及其数据结构
VFS使用一组数据结构来代表通用文件对象，这些数据结构类似对象。VFS中有4个主要的对象类型：
...
每个主要对象中都包含一个操作对象，描述了内核针对主要对象可以使用的方法。
...
```
超级块对象通过`alloc_super()`函数创建并初始化，在文件系统安装时，文件系统会调用该函数以便从磁盘读取文件系统超级块，并将其信息填充到内存中的超级块对象中。

## 13.6 超级块操作
`s_op`域指向超级块的操作函数表，由super_operations结构体表示，每一项都是指向一个超级块操作函数的指针。

## 13.7 索引节点对象
索引节点对象包含了内核在操作文件或目录时需要的全部信息。对于Unix风格的文件系统，这些信息可以直接从磁盘索引节点读入。
而没有索引节点的文件系统通常将文件的描述信息作为文件的一部分来存放。不管哪种方式，索引节点对象必须在内存中创建，以便于文件系统使用。
...
```
一个索引节点代表文件系统中的一个文件，也可以是设备或者管道这样的特殊文件。

## 13.8 索引节点操作
`inode_operations`域描述了VFS用以操作索引节点对象的所有方法，由文件系统实现。

## 13.9 目录项对象
VFS把目录当作文件对待，但是解析一个路径并遍历其分量比较复杂。因此，VFS引入了目录项对象。
每个dentry代表路径中的一个特定部分，比如`/bin/vi`中，`/`, `bin`和`vi`都属于目录项对象。前两个是目录，最后一个是文件。
```
struct dentry {
    atomic_t            d_count;        // 使用标记
    unsigned int        d_flags;        // 目录项标识
    spinlock_t          d_lock;         // 单目录项锁
    int                 d_mounted;      // 是否是登录点的目录项
...
```
目录项对象没有对应的磁盘数据结构，VFS都是根据字符串形式的路径名现场创建它。

### 13.9.1 目录项状态
1. 被使用：对应一个有效的索引节点，并且表明该对象存在一个或者多个使用者。
2. 未被使用：对应一个有效的索引节点，但是当前VFS并未使用它。该目录项对象仍然执行一个有效对象，被保留在缓存中以便需要时再次使用。
3. 负状态：没有对应的有效索引节点。因为对应的索引节点已经被删除，或者路径不再正确了。但是目录项仍然保留，以便快速解析以后的路径查询

### 13.9.2 目录项缓存
因为VFS遍历路径名中的所有元素并逐个解析成目录项对象是一件非常费力的事情，
因此，内核将目录项对象缓存在目录项缓存（dcache)，包括
+ 被使用的目录项链表
+ 最近被使用的双向链表：还有未被使用的和负状态的目录项对象
+ 散列表和相应的散列函数，用来快速的将给定路径解析为相关目录项对象
散列表由数组dentry_hashtable表示，每一个元素都是一个指向具有相同键值的目录项对象链表的指针。
dcache也在一定意义上提供对索引节点的缓存，也就是icache。和目录项对象相关的索引节点对象不会被释放

 ## 13.11 文件对象
文件对象表示进程已经打开的文件。由相应的open()系统调用创建，由close()系统调用撤销。
所有这些文件的相关调用都是文件操作表中定义的方法。一个文件可能对应多个文件对象，因为多个进程可以同时打开一个文件。

文件对象仅仅在进程观点上代表已打开的文件，它反过来指向目录项对象，再反过来指向索引节点。其实只有目录项对象才表示已打开的世纪文件。

类似于目录项对象，文件对象实际上也没有对应的磁盘数据。
...
+ file_struct：由进程描述符中的files目录项指向，所有与单个进程相关的信息都包含其中
+ fs_struct：由进程描述符的fs域指向，包含文件系统和进程相关信息
+ namespace：由进程描述符的mmt_namespace域指向
...
# 进程地址空间
12章介绍了内核如何管理物理内存，内核除了管理本身内存之外，还必须管理用户空间中进程的内存。
我们称这个内存为进程地址空间，也就是每个用户空间进程所看到的内存。
Linux采用虚拟内存技术，使得所有进程之间以虚拟方式共享内存。

## 15.1 地址空间
进程地址空间由进程可寻址的虚拟内存组成。每个进程都有一个32／64位的平坦（即独立的连续区间）地址空间。

现代采用虚拟内存的操作系统都使用平坦的地址空间，通常情况下，每个进程都有唯一的这种平坦地址空间。

尽管一个进程可以寻址4GB的虚拟内存（32位），但是并不代表有权限访问所有的虚拟地址。
在地址空间中，可以被访问的合法地址空间称为内存区域(08048000~0804c000)。进程只能访问有效内存区域内的内存地址。
...
struct mm_struct {
    ...
    unsigned long           start_data; // 数据开始地址
    unsigned long           end_data;   // 数据结束地址
    ...
    cpumask_t               cpu_vm_mask;    // TLB交换掩码
    mm_context_t            context;    // 体系结构特殊数据
    unsigned long           flags;      // 状态标志
    int                     core_waiters;   // 内核转储等待线程
    struct core_state       *core_state;    // 核心转储的至此
    spinlock_t              ioctx_lock;     // AIO IO链表锁
    struct hlist_head       ioctx_list;     // AIO IO链表
}
```
mmap和mm_rb这2个结构体描述的对象是相同的: 该地址空间的全部内存区域。

但是mmap以链表形式存放，而mm_rb以红黑树形式存放。一个方便高效的遍历所有元素，一个方便快速的搜索指定元素。
...
`fork()`利用`copy_mm()`复制父进程的内存描述苻给其子进程，
而子进程中的mm_struct结构体实际是通过`allocate_mm()`宏从`mm_cachep_slab`缓存中分配得到的。
通常，每个进程都有唯一的mm_struct结构体，即唯一的进程地址空间。

如果父进程希望和子进程共享地址空间，可以在调用`clone()`时，设置`CLONE_VM`标志，这样的进程称作线程。
其实，是否共享地址空间几乎是线程和进程的本质的唯一区别。

如果在创建线程时，即在clone()时设置CLONE_VM标志，内核就不再需要调用allocate_mm()，而仅仅在调用copy_mm()时将mm域指向父进程的内存描述苻即可。

### 15.2.2 撤销内存描述苻
进程退出时，内核会调用`exit_mm()`函数,执行一些常规的撤销工作，更新一些统计量。
其中，该函数会调用`mmput()`减少内存描述苻中的`mm_user`计数，如果mm_user变为0，将调用`mm_drop()`，减少`mm_count`计数，
如果`mm_count`变为0，调用`free_mm()`将mm_struct归还到mm_cachep_slab中。

### 15.2.3 mm_struct与内核线程
内核线程没有进程地址空间，也就没有内存描述苻。所以内核线程对应的进程描述符中的mm域为空。
这也是内核线程的真实含义 - 没有用户上下文

内核线程并不需要访问任何用户空间的内存，而且因为内核新城在用户空间没有任何页，所以实际上它们并不需要有自己的内存描述符和页表。
为了避免内核线程为内存描述符和页表浪费内存，内核线程将直接使用前一个进程的内存描述符。

当一个进程被调度时，该进程的mm域指向的地址空间被装载到内存，进程描述符中的active_mm域会被更新，指向新的地址空间。
内核线程因为没有地址空间，所以mm域为NULL，所有，当一个内核线程被调度时，内核发现mm域是NULL，就会保留前一个进程的地址空间，
随后内核更新内核线程对应的进程描述符中的active_mm域，使其指向前一个进程的内存描述符。

## 15.3 虚拟内存区域
由`vm_area_struct`描述，内存区域在linux内核中也经常称为虚拟内存区域(VMAs)。
`vm_area_struct`描述了指定地址空间内连续区间上的一个独立内存范围。内核将每个内存区域作为一个单独的内存对象管理，每个内存区域有一致的属性，
按照这样的方式，每一个VMA可以代表不同类型的内存区域，这种管理方式类似于使用VFS层的面向对象方法。

### 15.3.1 VMA标志
VMA标志是一种为标志，包含在vm_flags域中，标志了内存区域所包含的页面的行为和信息。和物理页的访问权限不同，VMA标志反映了内核处理页面所需要遵守的行为准则，
而不是硬件要求。

其中，VM_SEQ_READ标志按时内核应用程序对映射内容执行有序的读操作；这样，内核可以由选择的执行预读文件。

### 15.3.3 内存区域的树型结构和内存区域的链表结构
可以通过内存描述符的`mmap`和`mm_rb`域之一访问内存区域，这两个域独立的指向与内存描述符相关的全体内存区域对象，其实，它们包含完全相同的`vm_area_struct`
的指针，只是组织方法不同而已。

`mmap`使用链表连接所有内存区域对象，`mm_rb`使用红黑树连接所有内存区域对象。
链表用于遍历全部节点，红黑树用于在地址空间定位特定内存区域，所以同时使用了2种数据结构。

### 15.3.4 实际使用中的内存区域

## 15.4 操作内存区域
内核市场需要在某个内存区域上进行一些操作，比如某个指定地址是否包含在某个内存区域内，这类操作比较频繁，也是`mmap()`例程的基础。
...
内核使用do_mmap()创建一个新的线性地址空间。
`do_mmap()`会将一个地址区间加入到进程的地址空间中。
...
## 15.7 页表
虽然应用程序操作的对象是映射到物理内存上的虚拟内存，但是处理器直接操作的确实物理内存，
所以，当应用程序访问一个虚拟地址时，首先必须将虚拟地址转化成物理地址，然后处理器才能解析地址访问请求。
地址的转换工作需要通过查询页表完成。

概括的讲，地址转化需要将虚拟地址分段，使每段虚拟地址都作为一个索引项指向页表，而页表项指向下一级页表或者最终物理地址。

Linux使用三级页表完成地址转换。多级页表可以节约地址转换所需要占用的存放空间：
+ 顶级页表上页全局目录（PGD），包含了一个pgd_t类型(多数体系结构中等同于无符号长整数类型)数组。PGD中的表项指向二级目录的表项：PMD
+ 二级页表上中间页目录（PMD），是个pmd_t类型数组，其中表项指向PTE中的表项
+ 最后一级简称页表(PTE)，包含了pte_e类型的页表项，该页表项指向物理页面

每个进程都有自己的页表，内存描述苻的`pgd`域指向的就是进程的页全局目录。操作和检索页表时必须使用`page_table_lock`锁，以防止竞争条件.

几乎每次对虚拟内存的页面访问都必须先解析页表，从而得到物理内存中的对应得治，所以页表操作的性能非常关键，但是搜索内存中的物理地址速度有限，
所以多数体系结构实现了一个翻译后缓冲器(TLB)的一个将虚拟地址映射物理地址的硬件缓存。

2.6版对页表管理的主要改进：从高端内存分配部分页表。
2.6版本之后的可能改进包括写时拷贝的方式共享页表。


# 页高速缓存和页回写！！！
页高速缓存是Linux内核实现的磁盘缓存，它主要用来减少对磁盘的IO操作。具体来讲，是通过把磁盘中的数据缓存到物理内存中，把对磁盘的访问变为对物理内存的访问。

## 16.1 缓存手段
页高速缓存由内存中的物理页面组成，其内容对应磁盘上的物理块。页高速缓存大小可以动态调整。

当内核开始一个读操作（比如进程发起一个read()系统调用), 它首先会检查需要的数据是否在页高速缓存中，如果在，放弃访问硬盘，直接从内存中读取，即缓存命中。
如果不在缓存中，缓存未命中，那么内核必须调度块IO操作从磁盘去读取数据，然后将读来的数据放在页缓存中，于是后续的相同数据的读取都可以在内存中进行了。
注意，系统不一定是将整个文件缓存，也可以缓存一页或者几页

### 16.1.1 写缓存
写缓存一般实现成下面三种策略之一：
+ 不缓存：不去缓存任何写操作。直接跳过缓存，写到磁盘，还要使缓存数据失效。很少使用
+ 写透缓存：自动更新内存缓存，同时也更新磁盘文件，写操作会立刻穿透缓存存到磁盘。对保持缓存一致性有好处。
+ 回写(Linux采用)：写操作直接写到缓存，后端存储不会立即更新么事将页高速缓存中被写入的页面标记成“脏”，加入到脏页链表中。
然后一个进程周期性的将脏页链表中的页写回到磁盘，从而让磁盘中的数据和内存中最终一致

### 16.1.2 缓存回收
Linux缓存回收是通过选择干净页进行简单替换，如果缓存中没有足够的干净页，内核将强制地进行回写操作，以腾出更多干净可用页。
+ LRU 最近最少使用。LRU跟踪每个页的访问踪迹，以便能够回收最老时间戳的页。
+ 双链策略：活跃链表和非活跃链表，在活跃链表中的页面被认为是"热"的且不会被换出，在非活跃链表中的页则是可以被换出的。在活跃链表中的页面必须在其被访问时就处于非活跃链表中。
两个链表都被LRU规则维护. 如果活跃链表变得过多而超过了非活跃链表，那么活跃链表的头页面就重新移到非活跃链表中，以便能被再次回收。

## 16.2 Linux页高速缓存

### 16.2.1 address_space对象
在页高速缓存中的页可能包含了多个不连续的物理磁盘块，也正是由于页面中映射的磁盘块不一定连续，所以在页高速缓存中检查特定数据是否已经被缓存是件很困难的工作。
Linux页高速缓存的目标是缓存任何而基于页的对象，包括各类型的文件和各类型的内存映射。

Linux使用了一个新对象管理缓存项和页IO操作，address_space结构体。一个文件可以有多个虚拟地址，但是只能在物理内存有一份
```
struct address_space {
    ...
    struct address+space *assoc_mapping;    // 相关的缓冲
};
```

### 16.2.2 address_space操作
每个后备存储都通过自己的`address_space_operation`描述自己如何与页高速缓存交互。这里`readpage()`和`writepage()`两个方法最为重要。

一个页面的读操作包括的步骤:
1. 首先Linux内核试图在页高速缓存中找需要的数据: 用find_get_page()方法来完成
```
// mapping: address_space对象
// index: 偏移量
page = find_get_page(mapping, index)
```
2. 如果搜索的也不在高速缓存中，find_get_page()返回一个NULL，并且内核将分配一个新页面，将之前搜索的页加入到页高速缓存中
3. 最后，需要的数据从磁盘被读入，再被加入到页高速缓存，然后返回给用户

对于写操作而言，当页被修改了，VM仅需滴啊用`SetPageDirty(page)`, 内核晚些时候会通过`writepage()`方法把页写出。

### 16.2.3 基树
因为在任何页IO操作前内核都要检查页是否已经在页高速缓存了，所以这种频繁进行的检查必须迅速、高效。

页高速缓存通过2个参数：address_space对象加一个偏移量进行搜索，每个address_space对象都有唯一的基树(radix tree), 保存在page_tree结构体中。基树是一个二叉树，
只要制定了文件偏移量，就可以在基树中迅速检索到希望的页。

### 16.2.4 以前的页散列表
略

## 16.3 缓冲区高速缓存
独立的磁盘块通过块IO缓冲也要被存入页高速缓存。一个缓冲是一个物理磁盘块在内存的表示，缓冲的作用就是映射内存中的页面到磁盘块，这样一来页高速缓存在块IO操作时减少了磁盘访问，
因为它缓存磁盘块和减少块IO操作，所以这个缓存通常被称为缓冲区高速缓存。实际上没有作为独立缓存，而是作为页高速缓存的一部分实现。

## 16.4 flusher线程
在以下三种情况发生时，脏页被写回磁盘：
1. 当空闲内存低于一个特定的阈值，内核必须将脏页写回磁盘以便释放内存
2. 当脏页在内存中驻留时间超过一个特定的阈值时，内核必须将超时的脏页写回磁盘
3. 当用户进程调用sync()和fsync()系统调用时，内核会按要求执行回写动作

在2.6内核中，由一群内核线程(flusher线程)来执行这3种工作。
当空闲内存低于一个特定阈值时，内核便会调用flusher_threads()唤醒一个或多个flusher线程，随后flusher线程进一步调用函数bdi_writeback_all()开始将脏页写回。
直到满足以下两个条件:
1. 已有指定的最小数据的页被写回到磁盘
2. 空闲内存数超过阈值

为了满足第二个情况，flusher线程后台例程会被周期性的唤醒，将在内存中驻留时间过长的脏页写出。
