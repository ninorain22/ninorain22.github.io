...
```
##### canceler接口
是一个扩展接口，规定了取消通知的Context具体类型需要实现的接口
```go
...
##### cancelCtx
cancelCtx是一个实现了canceler和Context接口的具体类型
```go
...
##### timerCtx
timerCtx是一个实现了Context接口的具体类型，内部封装了cancelCtx实例类型，同时又一个dealline变量，用来实现定时退出通知

##### valueCtx
valueCtx是一个实现了Context接口的具体类型，内部封装了Context接口类型，同时封装了一个k/v存储变量


#### 5.3.3 API函数
构造Context取消树的根节点对象，根节点对象用作后续With包装函数的实参
```go
func Background() Context
func TODO() Context
```
...
每个M都有一个自己的管理堆栈g0, g0不指向任何可执行的函数，g0仅在M执行管理和调度逻辑时使用。在调度或系统调用时会切换到g0的栈空间。
...
然后绑定P个G运行。所以M和P不是一一对应的，M是按需分配，但在运行时会设置一个上限值(默认10000)
...
+ 所有类型通用方法：
    ...
    + Method(int) Method：通过索引值访问方法
    + MethodByName(string) (Method, bool)：通过方法名获取方法
    + PkgPath() string：返回类型的暴露就，如果类型是预声明类型或者未命名类型，返回空字符串
...
1. 如果a绑定了具体类型实例，返回a绑定的具体实例类型的信息
...
##### 底层类型和基础类型
基础类型时抽象的类型划分，底层类型时针对每一个具体的类型来定义的。
```go
type A struct {
	a int
}
...
```
3. 从Type到Value
Type里只有类型信息，所以无法直接从一个Type接口变量里获得实例V的Value，但是可以通过Type构建一个新的实例
```go
