...
1. 寻找合适的输入文件
    + 对于level 0层文件过多引发的合并或者由于level i层文件总量过大的合并，采用轮转方法选择输入文件，记录上一次该层合并文件的最大key，下一次选择这个key之后的首个文件
    + 错峰合并，起始输入文件为该查询次数过多的文件
2. 根据key重叠情况扩大输入文件集合
    + 在level i层，查找与起始输入文件有key重叠的文件，最终构成level i层的输入文件
    + 利用level i层的输入文件，在level i+1层寻找有key重叠的文件，构成level i、i+1层的输入文件
    + 最后利用两层的输入文件，在不扩大level i+1输入文件前提下，查找level i层有key重叠的文件，构成最终输入文件
3. 多路合并：将level i层的文件，与level i+1层文件中的数据项，按序整理输出到level i+1层若干个新文件中
4. 积分计算：使用记分牌来维护每一层文件的文件个数及数据总量信息，来挑选出下一个需要合并的层数。计分规则如下：
    + 对于0层文件，该层的分数为文件总数/4
    + 非0层文件，分数为文件数据总量/数据总量上限
将得分最高的层数记录，若超过1，则为下一次进行合并的层数
...
### Commit
每当完成一次major compaction或通过minor compaction或者重启阶段的日志replay新生成一个0层文件，都会触发leveldb进行一个版本升级
...
2. 创建一个空的version，利用manifest文件中的session record依次做replay，还原出一个新的version。
...
### 多版本并发控制
1. sstable文件是只读的，每次compaction都只是对若干个sstable进行多路合并后创建新的文件，故不会影响在某个sstable文件读操作的正确性
